...
poster_empty = kodi_utils.translate_path('special://home/addons/script.B99art/resources/media/box_office.png')
fanart_empty = kodi_utils.translate_path('special://home/addons/plugin.video.B99/fanart.png')

class B99Player(kodi_utils.xbmc_player):
	def __init__ (self):
	...
	def run(self, url=None, media_type=None):
		...
		try:
			...
			self.meta = json.loads(kodi_utils.get_property('B99_playback_meta'))
			...
			else: bookmark = self.bookmarkB99()
	...
	def media_watched_marker(self):
		...
		try:
			if self.current_point >= self.set_watched:
				...
				kodi_utils.clear_property('B99_nextep_autoplays')
				kodi_utils.clear_property('B99_random_episode_history')
...
class Subtitles(kodi_utils.xbmc_player):
	...
	def get(self, query, imdb_id, season, episode):
		...
		sub_filename = 'B99Subs_%s_%s_%s' % (imdb_id, season, episode) if season else 'B99Subs_%s' % imdb_id
