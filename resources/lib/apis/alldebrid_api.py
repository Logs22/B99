...
class AllDebridAPI:
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('B99_ad_user_cloud',))
				kodi_utils.clear_property('B99_ad_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('B99_ad_valid_hosts',))
				kodi_utils.clear_property('B99_ad_valid_hosts')
				dbcon.commit()
				dbcon.close()
