...
timeout = 20.0
session = make_session('https://api.real-debrid.com')
...
class RealDebridAPI:
	...
	def user_cloud(self):
		string = 'B99_rd_user_cloud'
		url = 'torrents'
		return cache_object(self._get, string, url, False, 0.5)

	def downloads(self):
		string = 'B99_rd_downloads'
		url = 'downloads'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_info(self, file_id):
		string = 'B99_rd_user_cloud_info_%s' % file_id
			...
			for item in torrent_files:
				try:
					...
					if season:
						...
						if match: break
				except Exception as e: logger('loop exception', str(e))
			if match:
	...
	def add_uncached_torrent(self, magnet_url, pack=False):
		...
		if status == 'magnet_conversion':
			...
			while status == 'magnet_conversion' and progress_timeout > 0:
				progressDialog.update(progress_timeout, line % (line1, line2, line3))
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT data FROM maincache WHERE id=?""", ('B99_rd_user_cloud',))
				try: 
				...
				if not user_cloud_success:
					dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('B99_rd_user_cloud',))
					clear_property("B99_rd_user_cloud")
					for i in user_cloud_info_caches:
						dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('B99_rd_user_cloud_info_%s' % i,))
						clear_property("B99_rd_user_cloud_info_%s" % i)
					dbcon.commit()
					user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('B99_rd_downloads',))
				clear_property("B99_rd_downloads")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('B99_rd_valid_hosts',))
				clear_property('B99_rd_valid_hosts')
				dbcon.commit()
				dbcon.close()
